...
pub fn follow_routes() -> Router<sqlx::Pool<sqlx::Sqlite>> {
    Router::new()
        .route("/user/{id}/{id}", get(get_follow_user))
        .route("/media/{id}/{id}", get(get_follow_media))
        .route("/list/{id}/{type}", get(get_follow_list))
        .route("/user", post(post_follow_user).delete(delete_follow_user))
        .route(
            "/media",
            post(post_follow_media).delete(delete_follow_media),
        )
}
...
async fn post_follow_user(
    State(pool): State<SqlitePool>,
    Json(input): Json<PostFollowUserRequest>,
) -> impl IntoResponse {
...
async fn post_follow_media(
    State(pool): State<SqlitePool>,
    Json(input): Json<PostFollowMeidaRequest>,
) -> impl IntoResponse {
    let sql = "INSERT INTO following_user (user_id, media_id) VALUES ($1, $2)";
    match query(&sql)
        .bind(input.user_id)
        .bind(input.media_id)
        .execute(&pool)
